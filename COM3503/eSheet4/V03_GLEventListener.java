...
public class V03_GLEventListener implements GLEventListener {
  ...
  public void render(GL3 gl) {
    ...
    for (int i=-2; i<3; ++i) {
      for (int j=-2; j<3; ++j) {
      	  if(i%3==0){
				Mat4 model = getModelMatrix(2f*i, 2f*j, 1);
				Mat4 mvpMatrix = Mat4.multiply(perspective, Mat4.multiply(view, model));
				shader.setFloatArray(gl, "mvpMatrix", mvpMatrix.toFloatArrayForGLSL());    
		  	}
		  	else if(i%3 == 1){
				Mat4 model = getModelMatrix(2f*i, 2f*j, 2);
				Mat4 mvpMatrix = Mat4.multiply(perspective, Mat4.multiply(view, model));
				shader.setFloatArray(gl, "mvpMatrix", mvpMatrix.toFloatArrayForGLSL());    
			}
			else{
				Mat4 model = getModelMatrix(2f*i, 2f*j, 3);
				Mat4 mvpMatrix = Mat4.multiply(perspective, Mat4.multiply(view, model));
				shader.setFloatArray(gl, "mvpMatrix", mvpMatrix.toFloatArrayForGLSL());    
			}
  ...
  private Mat4 getModelMatrix(float i, float j, int type) {
    ...
    model = Mat4.multiply(model, Mat4Transform.rotateAroundZ(angle));
    model = Mat4.multiply(model, Mat4Transform.translate(i, 0, j));
    }
    return model;
  }

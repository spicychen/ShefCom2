...
public class Hand{
	...
	private TruncatedCone ringShape;
	
	//the parameters you can change to adjust the size of the hand
	//change overall_ratio will change whole size of the hand.
	private float overall_ratio = 1.0f, arm_length = 5.0f*overall_ratio,
					arm_width = 1.5f*overall_ratio, palm_length = 2.0f*overall_ratio,
					 palm_width = 1.8f*overall_ratio,
					thumb_length = 0.8f*overall_ratio, index_length = 0.8f*overall_ratio,
	...
	//how each part should scale
	//ring and True ring are different thing, ring means ring finger, TrueRing is the real ring on the finger.
	...
	//the Nodes used to animate the hand
	...
	// collection of above transform nodes
	...
	//can be used to adjust the size of spotlight
	...
	public void initialise(GL3 gl, Light[] lights, Camera camera){
		...
		//ring texture
		int[] ring_texture0 = TextureLibrary.loadTexture(gl, "textures/jup0vss1.jpg");
		int[] ring_texture1 = TextureLibrary.loadTexture(gl, "textures/jup0vss1_specular.jpg");
		
		//palm and finger texture
		int[] cube_texture0 = TextureLibrary.loadTexture(gl, "textures/creeper.jpg");
		int[] cube_texture1 = TextureLibrary.loadTexture(gl, "textures/creeper_specular_g.jpg");
		
		//arm texture
		int[] arm_texture0 = TextureLibrary.loadTexture(gl, "textures/mar0kuu2.jpg");
		int[] arm_texture1 = TextureLibrary.loadTexture(gl, "textures/mar0kuu2_specular.jpg");
		
		//create shape
		Mat4 m = new Mat4(1);
		cube0 = new Cube(gl, cube_texture0, cube_texture1);
		cube0.setLight(lights);
		cube0.setCamera(camera);
		
		//thumb cube, where it is darker
		cube1 = new Cube(gl, cube_texture0, cube_texture1);
		cube1.darker();
		cube1.setLight(lights);
		cube1.setCamera(camera);
		
		sphere = new Sphere(gl, arm_texture0, arm_texture1);
		sphere.setLight(lights);
		sphere.setCamera(camera);
		
		ringShape = new TruncatedCone(gl, ring_texture0, ring_texture1,0.5,0.5);
		ringShape.setLight(lights);
		ringShape.setCamera(camera);
		
		//create hierachy nodes create
		//Arm
		NameNode arm = new NameNode("arm");
		m = Mat4Transform.translate(0,0.5f,0);
		//offset is where it should relate to its origin when doing other transformation
		...
		//same technique as creating arm
		//Palm
		...
		//translate to the corresponding position in the world
		...
		//+0.1 here to have an indentaion between fingers
		...
		//make the thumb rotate in an angle initially
		...
		MeshNode trueRingShape = new MeshNode("True Ring shape", ringShape);
		//ring end
	...
	/**
	* gets the ring finger transform matrix which contains the information of ring finger direction
	* @return matrix of ring finger
	*/
	public Mat4 getRingDir(){
		return ring1Rotate.getTransform();
	}
	
	/**
	*gets the ring finger position in the world
	* @return vec3 that represents the position of the ring finger
	*/
	public Vec3 getRingPos(){
		Mat4 m = Mat4.multiply(ring1Rotate.getTransform(), Mat4Transform.translate(0,-0.6f*overall_ratio,-hand_width));
		return Mat4.Vec4multiply(m, new Vec3(0,0,0));
	}
	
	public void update(){
		//update any change to the hand
		hand.update();
	}
	
	public void resetRotation(){
		
		//reset all the rotation of the hand
		for(int i=1; i<rotateNodes.length; i++)
			rotateHandPart(i,0,'Y');
			
		//including the thumb initial rotation
		rotateHandPart(17,60.0f,'Z');
	}
	
	/**
	 * rotate a part of the hand with an angle in a direction
	 * @param int handpartid
	 * @param float amount_of_angle
	 * @param char rotation_direction (XYZ)
	...
	/**
	 * animate a part of the hand with target angle in a direction
	 * @param int handpartid
	 * @param double elapsed time
	 * @param double start time
	 * @param double end time
	 * @param float total amount_of_angle
	 * @param char rotation_direction (XYZ)
	 * @param double the time where it should continue
	 * @param double animate speed
	...
	public void performAction(double time){
		//you can set the timer here to make different animation
		if (time<2.0)
			ASL_J(time);
		else if(time<4.0)
			release_J(time-2.0);
	...
	//gesture 'I Love You'
	public void IOU(double time){
		double speed1 = 1.0;
		
		//pinkie rotate
		foldHandPart(14,time,0.0,1.0,45.0f,'Z',0.0,speed1);
		
		//ring rotate
		foldHandPart(11,time,0.0,1.0,90.0f,'X',0.0,speed1);
		foldHandPart(12,time,0.5,1.5,90.0f,'X',0.5,speed1);
		foldHandPart(13,time,0.5,1.5,90.0f,'X',0.5,speed1);
		
		//middle rotate
		foldHandPart(8,time,0.0,1.0,90.0f,'X',0.0,speed1);
		foldHandPart(9,time,0.5,1.5,90.0f,'X',0.5,speed1);
		foldHandPart(10,time,0.5,1.5,90.0f,'X',0.5,speed1);
		
		//index rotate
		foldHandPart(5,time,0.0,1.0,-30.0f,'Z',0.0,speed1);
	...
	//set the perspective of the shape
	public void updatePerspectiveMatrices(Mat4 perspective) {
		...
		ringShape.setPerspective(perspective);
	}

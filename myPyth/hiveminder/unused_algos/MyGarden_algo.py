...
def cal_score(board_width,board_height,hives,flowers,inflight,turn_num,move):
	...
	fl_sc = flower_sco([f[0] for f in futureflowers],seedsx)
	...
	if move is not None:
		...
		bee = inflight[bee_id]
		core = futurehives[0]
		if action == "create_hive" and good_address(bee[1:3]) and not around_core(bee[1:3],core):
			states.append(("plant",0))
		elif action == "flower" and (not good_address(bee[1:3]) or around_core(bee[1:3],core)):
			states.append(("plant",0))

...
def cal_score(board_width,board_height,hives,flowers,inflight,turn_num,move):
	...
	if move is not None:
		...
		cmd = move['command']
		ifl = inflight[ett]
		if cmd == "create_hive" and (good_address(ifl[1:3]) and not around_core(ifl[1:3],futurehives_loc[0])):
			states.append(("plant",0))
		if cmd == "flower" and (not good_address(ifl[1:3]) or around_core(ifl[1:3],futurehives_loc[0])):
			states.append(("plant",0))
	...
	queenbees = [abstract_bee.to_json()[1:3] for bee_id, abstract_bee in futureinflight.items() if abstract_bee.to_json()[0]=="QueenBee"]
	if queenbees:
		for qb in queenbees:
			states.append(("centre",(-distance_between_hex_cells(qb,[4,3])*0.1)))
			states.append(("indanger",-neighbour_tiles(qb)))

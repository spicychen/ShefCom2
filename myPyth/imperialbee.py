...
def algo(board_width, board_height, hives, flowers, inflight, crashed,
    ...
    for typ, nb in zip([BEE, QUEEN, SEED],
                       [len(bee_ids), len(queen_ids), len(seed_ids)]):
        for v, a in product(
...
def start_game(board_width, board_height, hives, flowers, players, player_id,
    ...
    global bee_nectar_capacity, nectar_score_factor, flower_score_factor, \
        ...
        queen_bee_nectar_threshold, \
        nb_bee_states, bee_state_to_idx, bee_T, \
        nb_qs_states, qs_state_to_idx, qs_T, \
        bee_V, queen_V, seed_V, nectar_mask, success_prob
...
def compute_value_function(hives, flowers, turn_num):

    global bee_real_T, queen_real_T, \
        bee_V, queen_V, seed_V, \
        bee_R, queen_R, seed_R, \
        bee_policy, queen_policy, seed_policy, \
        bee_idle_waste, queen_idle_waste, seed_idle_waste, \
        bee_flowers_boost
    ...
    bee_real_T[bee_hives_mask] = nb_bee_states
    ...
    queen_real_T[qs_hives_mask] = nb_qs_states
    ...
    queen_R = dead_bee_score_factor * qs_hives_mask
...
def policy(bee_states, queen_states, seed_states):
    action = None
    max_waste = 0
    for typ, states, idle_waste, policy in zip(
        [BEE, QUEEN, SEED], [bee_states, queen_states, seed_states],
        [bee_idle_waste, queen_idle_waste, seed_idle_waste],
        [bee_policy, queen_policy, seed_policy]):
        if len(states):
...
def run_simulation(action, bee_states, bee_energies, queen_states,
    ...
    while steps < max_sim_steps and (len(bee_states) or len(queen_states) or
        ...
        crashed, dead_bees = detect_crashes(next_bee_states, bee_energies,
                                            next_queen_states, queen_energies,
                                            next_seed_states)
        ...
        # Prepare next action
        action = policy(bee_states, queen_states, seed_states)
...
def make_command(action, inflight, bee_ids, queen_ids, seed_ids):
    # Translate action into command
    if action is None:
        return None

    typ, v, a = action
    voland_id = {BEE: bee_ids, QUEEN: queen_ids, SEED: seed_ids}[typ][v]
    if a == SECONDARY:
        command = {QUEEN: "create_hive", SEED: "flower"}[typ]
    else:

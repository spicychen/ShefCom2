...
class Score:
    ...
    def print_measure1_summary(self,config):
        if self.total_retrieved > 0:
            self.precision = float(self.total_relevant_retrieved)/self.total_retrieved
        else: 
            self.precision = 0
        if self.total_relevant > 0: 
            self.recall = float(self.total_relevant_retrieved)/self.total_relevant
        else:
            self.recall = 0
...
if __name__ == '__main__':
    ...
    for i in range(1,11):
        ...
        precision_log.append(scorer.precision)
        recall_log.append(scorer.recall)
        F_measure.append(scorer.fmeasure)
    X = range(1,11)
    p.plot(X,precision_log,'bo-', label='precision')
    p.plot(X,recall_log,'r*-', label='recall')
    p.plot(X,F_measure,'g-', label='F measure')
    p.legend(loc= 'upper right')
    p.title('tfidf method with stop with stem')
    p.show()
    #response = Response(config,key)
    #scorer = Score(config,key,response)
    #scorer.print_measure1_summary(config)

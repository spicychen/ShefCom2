...
public class CarSales {
    public static void main (String[] args) throws Exception {
    	...
    	for (int i=0; i<nCars; i++) {
    		...
    		cars[i] = new Car(model, colour, price, mileAge, sold);
    	}

    	// Finally, print each of the cars in the array
    	int idealPrice = new EasyReader().readInt("what price do you want?");
    	for (int i=0; i<nCars; i++) {
    		if(cars[i].getPrice() <= idealPrice){
    			System.out.println("CarNo.: "+(i+1)+" "+cars[i]);
    			existCar = true;
    		}
    	}
    	int CarNo;
    		...
    		if(existCar)
    			CarNo = new EasyReader().readInt("enter the CarNo to buy car:");
    		else
    			System.out.println("Sorry, no such car.");
